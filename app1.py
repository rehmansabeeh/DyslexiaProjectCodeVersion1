...
def result():
    ...
    for element in user_text_tokens:
        ...
        for document in urdu_response:
            if element == document['correctword']:
                count = count + 1  
        if count >0:     
            final_results.append({"user_input": element, "found_count_in_db": count,"status": True,"user_input_length" : len(element), "suggested_word": element, "comments": "", "edit_distance":0, "found_count_in_worforms": 0})      
        else: 
            final_results.append({"user_input": element, "found_count_in_db": count,"status": False,"user_input_length" : len(element), "suggested_word": "", "comments": "",  "edit_distance":-1, "found_count_in_worforms": 0})
            ...
            for urdu_doc in urdu_response:
                for urdu_key in urdu_doc:
                    if not (urdu_key == "_id"):
                            ...
                            med = editDistance(element["user_input"], urdu_doc[urdu_key], len(element["user_input"]), len(urdu_doc[urdu_key]) )
    ...
    for element in final_results:
        if element["status"] == True:
            s = s +"<span style='display:inline; color:green'>"+ element["user_input"]+"</span>,"
        else:
            s = s +"<span style='display:inline; color:red'>"+ element["user_input"]+"</span>,"


    suggested_word_input = ""
    for element in final_results:
        suggested_word_input = suggested_word_input +"<span style='display:inline; color:black'>"+ element["suggested_word"]+"</span>,"
...
def loginverification():
    if request.method == 'POST':
        ...
        if login_user:
            ...
            print(bcrypt.hashpw(request.form['psw'].encode('UTF-8'), login_user['password']))
